...
int main(int argc, char const *argv[])
{
	...
	// info.rtuadd=11;
	// info.rtutype=RTU_TYPE_BED;   //RTU_TYPE_BED    
	...
	/************************4 创建 、修改 任务 ***********************************/
		...
		// taskinfo.hostadd=354353152;
		// taskinfo.taskid=11;
		// strcpy(taskinfo.taskname,"setup");
		// taskinfo.task_bind_num=4;
		// get_sys_time(taskinfo.taskregtime);
		// insert_db_taskinfo(taskinfo);
	/*********************5 添加  编辑应用设备 ****************************/
	 //   AppRtuInfo info;
	...
	/*************1 主机信息查询*******/
	HostRegInfo hostinfo;
	int ret=select_db_hostinfo(&hostinfo,"crazyman");
	if(ret == 0)
	{
		printf("%d\n", hostinfo.hostadd);
		printf("%s\n", hostinfo.hostpwd);
