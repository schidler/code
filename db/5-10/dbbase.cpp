...
{
	NativeRtuInfo rtuinfo=*prtuinfo;
	ostringstream ostr;
	string setstr;
	string wherestr;
	string tablename=get_table_name(type);
	...
	if(type == RTU_TYPE_SWITCH || type ==RTU_TYPE_CTRLBOX || type == RTU_TYPE_PLUG)
	{
		/* 处理开关控制等帧 */
		relayval=rtuinfo.val[0];
		ostr.str("");
		ostr << "Relay1Val=" << (relayval & 0x03) << ",Relay2Val=" << ((relayval >> 2) & 0x03) << ",Relay3Val=" << ((relayval >> 4) & 0x03) << ",Relay4Val=" << ((relayval >> 6) & 0x03);
		setstr = ostr.str();

	}
	else if(type == RTU_TYPE_RGB)
	{
		for (Int16 i = 0; i < 4;i++)
		{
			relayval = rtuinfo.val[i];
			printf("%d\n", relayval);
			if (!setstr.empty())
			{
				setstr.append(",");
...
/*int update_string(const string &setstr, const string &wherestr,Uint8* val)
  {
