...
{
		Uint32 ip;
	//GetIpByHostAdd(135004207, ip);
	  GetHostCount();
	 int count=findHostname("HR8000_201508TEST02");
	 printf("wjj test count=%d\n",count);
	 HostRegInfo hostinfo;
	 int ret=select_db_hostinfo(&hostinfo,"HR8000_201508TEST02");
	 if(ret == 0)
	 {
	 	printf("%d\n", hostinfo.hostadd);
	 	printf("%s\n", hostinfo.hostpwd);
     ...
     int nret=update_db_safetyrtu(safetyrtu);
	 printf("ret1111111111111 :%d\n",nret);
	 HostRegInfo hostinfo2;
	 hostinfo2.hostip=0xfffffffff;
	 strcpy(hostinfo2.hostname,"HR8000_201508TEST02");
	 update_db_host(hostinfo2);
	 RtuRegInfo info;
	 info.hostadd=354353149;
	 info.rtuadd=12;
	 info.rtutype=RTU_TYPE_SWITCH;   //RTU_TYPE_BED   RTU_TYPE_IRPROBE  RTU_TYPE_LIGHTDET 
	 get_sys_time(info.rturegtime);
	 info.rtustate=RTU_STATE_VALID;
	 //insert_db_rtu(info);
		  HostRegInfo hostinfo3;
		  sprintf(hostinfo3.hostname,"crazygeek");
		  sprintf(hostinfo3.hostpwd,"123456");
		  hostinfo3.hostadd=354353149;	
		  get_sys_time(hostinfo3.hostregtime);
		  hostinfo3.hoststate=RTU_STATE_VALID;
		  hostinfo3.hostip=19216801;
		  get_sys_time(hostinfo3.lastcomtime);
//		  insert_db_host(hostinfo3);
...
int main(int argc, char const *argv[])
{
	...
	/************1  更新常规设备   控制  绑定****************  */
	 ...
	 // RtuRegInfo info22;
	 // info22.hostadd=354353149;
	 // info22.rtuadd=11;
	 // info22.rtutype=RTU_TYPE_SWITCH;
	 // get_sys_time(info22.rturegtime);
	 // info22.rtustate=RTU_STATE_VALID;

	 // rtuinfo.rtuinfo=info22;
	 // rtuinfo.relayinfo[0].val=(2 & 0x03);
	 // rtuinfo.relayinfo[1].val=(2 & 0x03);
	 // rtuinfo.relayinfo[2].val=(2 & 0x03);
	 // rtuinfo.relayinfo[3].val=(2 & 0x03);
	 // // rtuinfo.bind_num=2;
	 ...
	 // NativeRtuInfo rtuinfo2;
	 // memset(&rtuinfo2,0,sizeof(rtuinfo2));
	 // RtuRegInfo info11;
	 // info11.hostadd=354353149;
	 // info11.rtuadd=12;
	 // info11.rtutype=RTU_TYPE_SWITCH;
	 // get_sys_time(info11.rturegtime);
	 // info11.rtustate=RTU_STATE_VALID;

	 // rtuinfo2.rtuinfo=info11;
	 // rtuinfo2.relayinfo[0].val=(2 & 0x03);
	 // rtuinfo2.relayinfo[1].val=(2 & 0x03);
	 // rtuinfo2.relayinfo[2].val=(2 & 0x03);
	 // rtuinfo2.relayinfo[3].val=(2 & 0x03);

		//  for (int i = 0; i < 10; ++i)
	 // {

		//  	update_db_nativertu(rtuinfo,0);
		//  	update_db_nativertu(rtuinfo2,0);	 		
 ...
 //    rtuinfo.detectorinfo.upval=1000;
	...
	// printf("host count:%d\n",count);
		...
		// 
		// 
	pthread_create(&main_tid, NULL, func, NULL); //创建线程
	pthread_create(&main_tid, NULL, func, NULL); //创建线程
	pthread_create(&main_tid, NULL, func, NULL); //创建线程
	pthread_create(&main_tid, NULL, func, NULL); //创建线程
	pthread_create(&main_tid, NULL, func, NULL); //创建线程
