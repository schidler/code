...
int insert_db_sceneinfo(SceneInfo & sceneinfo)
{
	...
	if(sceneinfo.rtubind == NULL)
	{
		LOG_DEBUG(" RtuBind is NULL...\n");
		return 0;
	}
	colstr = "UniqueIndex,HostAdd,SceneID";
	ostr.str("");
	ostr << UniqueIndex << "," << sceneinfo.hostadd << "," << sceneinfo.sceneid;
	valstr = ostr.str();
...
int insert_db_taskinfo(TaskInfo & taskinfo)
{
	...
	if(taskinfo.rtubind == NULL)
	{
		LOG_DEBUG(" insert_db_taskinfo RtuBind is NULL...\n");
		return 0;
	}
	colstr = "UniqueIndex,HostAdd,TaskID";
	ostr.str("");
	ostr << uniqueindex << "," << taskinfo.hostadd << "," << taskinfo.taskid;
	valstr = ostr.str();
