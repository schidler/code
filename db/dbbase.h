...
#define MAX_SQLNAME_LEN 256                 ///< 数据库主机长度
...
typedef struct  
{
	char hostname[MAX_SQLNAME_LEN];      ///<  数据库主机名
	char username[MAX_SQLUSERNAME_LEN];  ///<  数据库用户名
	char password[MAX_SQLPASSWD_LEN];    ///<  数据库密码
	char dbname[MAX_SQLNAME_LEN];        ///<  数据库名
...
typedef struct 
{
	...
	Uint16 type;        ///<  绑定类型  
	Uint32 add;         ///<  绑定设备/ 情景地址 
	Uint32 oper;          ///<  绑定设备继电器1-4的操作  4个字节
	Uint32 delay;      ///<  延时
}RtuBind;
...
typedef struct 
{
	...
	Uint32 taskrtunum;  ///< 绑定设备个数
	RtuBind* rtubind;    ///< 绑定的设备信息
}TaskInfo;
