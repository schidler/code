...
{
	RtuRegInfo rtuinfo;       //设备头信息 
	Uint16 valtype;           // 探测 类型  0x01光照，0x02可燃气危险指数，0x03可燃气浓度，0x04温湿度空气质量，0x05湿敏告警
	/**
		光照强度值（2）：0~4000lux
		可燃气危险指数（2）：0~100.00%，上送时*100，接收时/100
		可燃气浓度（2）：0~65535ppm
		温湿度空气质量（6）：包含以下三个值 
		温度（2）：-20.0~70.0℃，上送时*10，接收时/10，传输有符号数
		湿度（2）：0.0~100.0%，上送时*10，接收时/10
		空气质量（2）：0.0~1000.0，上送时*10，接收时/10
		湿敏告警（1）：非零值
	*/
	char   values[6];
}DetectorRtu;
...
int update_db_host(HostRegInfo & hostinfo,Uint8 mode,char* oldhostname);
