 ...
 *  更新主机信息表  
 * @Param: hostinfo  
 * @Param: mode  模式：0  更新主机ip ,连接时间  1  修改主机名，密码  
 * @Param: oldhostname  原主机名   用于模式1 
 *    更新信息： 传入  hostname ,hostip   修改主机 ： 传入主机名，密码  ,原主机名
 * Returns: 
 */
int update_db_host(HostRegInfo & hostinfo,Uint8 mode,char* oldhostname)
{
	...
	{
		ostr.str("");
		ostr << "HostName='" << oldhostname << "'";
		wherestr = ostr.str();
		ostr.str("");
		ostr << "HostName='" << hostinfo.hostname << "',HostPwd='" << hostinfo.hostpwd << "'";
		setstr = ostr.str();
	}
