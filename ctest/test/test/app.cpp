...
CEpollClient::CEpollClient(int iUserCount, const char* pIP, int iPort)
{
        ...
        for(int iuserid=0; iuserid<iUserCount ; iuserid++){
                ...
                sprintf(m_pAllUserStatus[iuserid].cSendbuff, " i am app %d.....\r\n",iuserid);
...
{
		static int count=0;
        usleep(1);//此处控制发送频率，避免狂打日志，正常使用中需要去掉
        int isendsize = -1;
        if( 1/*CONNECT_OK == m_pAllUserStatus[iUserId].iUserStatus || RECV_OK == m_pAllUserStatus[iUserId].iUserStatus*/)
        {
...
{
	CEpollClient *pCEpollClient = new CEpollClient(20000, "192.168.0.73", 9881);
